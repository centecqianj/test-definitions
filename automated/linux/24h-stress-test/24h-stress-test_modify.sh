...
cleanup_oom()
{
    if [ -n "${kernel_log:-}" ] && kill -0 "$kernel_log" 2>/dev/null; then
        kill "$kernel_log" 2>/dev/null || true
        wait "$kernel_log" 2>/dev/null || true
    fi
}
...
stress_oom()
{
    ...
    # Create a result file early so that LAVA can still collect something even
    # if the test is interrupted by OOM pressure.
    mkdir -p "$OUTPUT"
    : > "$RESULT_FILE"
    ...
    echo -17 > "/proc/${kernel_log}/oom_score_adj" 2>/dev/null || true
    # Try to keep the shell and result collection path alive as much as possible.
    ...
    cleanup_oom
    oom_number=$(grep -Ec "Out of memory: (Kill|Killed) process" "$OUTPUT/stress_oom_kern.log" 2>/dev/null || true)
    if [ "$oom_number" -gt 0 ]; then
        echo "oom-killer activated $oom_number times within $DURATION seconds"
        report_pass "stress-oom-test"
    else
        echo "Failed to activate oom-killer."
        report_fail "stress-oom-test"
    fi

    # Preserve a minimal result file so LAVA still gets a completion artifact
    # even when the workload is aggressively memory-stressing.
    {
        echo "stress_oom_rc=${stress_rc}"
        echo "oom_kill_events=${oom_number:-0}"
    } >> "$RESULT_FILE"
}
